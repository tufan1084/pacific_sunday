...
interface Pick {
  ...
  score: string;
  thru: string;
  scoreType: "negative" | "positive";
}

interface YourPicksLiveProps {
  picks: Pick[];
}

export default function YourPicksLive({ picks }: YourPicksLiveProps) {
  if (picks.length === 0) {
    return null;
  }
      ...
      {picks.map((pick, i) => (
        <div key={i}>
          <div className="flex items-center" style={{ padding: "12px 16px", gap: "12px" }}>
            ...
            <span style={{ fontSize: "clamp(13px, 1.5vw, 16px)", fontWeight: 500, color: pick.scoreType === "negative" ? "#EF4444" : pick.score === "E" ? "#FFFFFF" : "#4ADE80", flexShrink: 0 }}>
              {pick.score}
            </span>
            <span style={{ color: "#FFFFFF", fontSize: "clamp(13px, 1.5vw, 16px)", flexShrink: 0, minWidth: "16px", textAlign: "right" }}>{pick.thru}</span>
          </div>
          {i < picks.length - 1 && <div style={dividerStyle} />}
        </div>
      ))}
    </div>
  );
