...
export default function ProfileCard({ profileData }: ProfileCardProps) {
  ...
  const golfPassport = profileData?.golfPassport;
  ...
  const badges = [
    { text: userStats?.userRank ? `Rank: #${userStats.userRank}` : 'Rank: —', clickable: false },
    { text: `${userStats?.userPoints?.toLocaleString() || 0} pts`, clickable: true },
    { text: `${userStats?.weeksRegistered || 0} weeks`, clickable: false },
  ];
    ...
    <div style={{ backgroundColor: "#13192A", borderRadius: "5px", padding: "20px", fontFamily: "var(--font-poppins), sans-serif" }}>
      ...
      <div className="flex flex-wrap gap-2">
        {badges.map((badge, i) => (
          <div
            key={i}
            onClick={() => badge.clickable && router.push('/points-history')}
            style={{
            ...
            onMouseEnter={(e) => {
              if (badge.clickable) {
                ...
                e.currentTarget.style.boxShadow = "0 2px 8px rgba(232,201,106,0.3)";
              }
            }}
            onMouseLeave={(e) => {
              if (badge.clickable) {
