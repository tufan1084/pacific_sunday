...
export default function NotificationsSection() {
  const [notifications, setNotifications] = useState({
    fantasyReminders: false,
    liveScoreUpdates: true,
    communityMentions: false,
    achievementAlerts: false,
    h2hChallengeAlerts: false,
    majorEventAlerts: true,
  });
  ...
  const notificationItems = [
    { key: 'fantasyReminders', title: 'Fantasy Reminders', desc: 'Pick deadlines & lock alerts' },
    { key: 'liveScoreUpdates', title: 'Live Score Updates', desc: 'Round-by-round alerts' },
    { key: 'communityMentions', title: 'Community Mentions', desc: 'Replies to your posts' },
    { key: 'achievementAlerts', title: 'Achievement Alerts', desc: 'New badge unlocks' },
    { key: 'h2hChallengeAlerts', title: 'H2H Challenge Alerts', desc: 'New challenges & results' },
    { key: 'majorEventAlerts', title: 'Major Event Alerts', desc: 'Double points reminders' },
  ];
  ...
  return (
    <div style={{ backgroundColor: '#13192A', borderRadius: '8px', padding: 'clamp(16px, 2vw, 20px) clamp(20px, 2.5vw, 24px)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
        ...
        <span style={{ color: '#E8C96A', fontSize: '16px', fontWeight: 500 }}>Notifications</span>
      </div>
      <div style={{ height: '1px', backgroundColor: 'rgba(255, 255, 255, 0.08)', margin: '0 -24px 12px -24px' }} />

      <div>
        {notificationItems.map(({ key, title, desc }, index) => (
          <div key={key}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: index === 0 ? '0' : '10px', paddingBottom: index === notificationItems.length - 1 ? '0' : '10px' }}>
              <div>
