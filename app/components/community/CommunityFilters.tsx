...
export default function CommunityFilters({
  ...
  // Only attach the scroll listener while a dropdown is actually open. Before,
  // it ran on every scroll tick (120/s on iOS) and called setState even when
  // both dropdowns were already closed — one of the audited iPhone hot paths.
  useEffect(() => {
    if (!teamOpen && !tabOpen) return;
    const main = document.querySelector("main");
    if (!main) return;
    const onScroll = () => { setTeamOpen(false); setTabOpen(false); };
  ...
  return (
    <div className="flex items-center gap-2" style={{ marginBottom: "8px", minWidth: 0 }}>
      {/* Team dropdown */}
